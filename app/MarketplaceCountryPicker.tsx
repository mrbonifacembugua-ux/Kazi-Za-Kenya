...
function globalCountries(): Country[] {
  ...
  for (let a = 65; a <= 90; a += 1) {
    for (let b = 65; b <= 90; b += 1) {
      ...
      if (name && name !== code && !name.startsWith("Unknown Region")) result.push({ code, name });
    }
  }
  return result;
}
...
export default function MarketplaceCountryPicker() {
  ...
  useEffect(() => {
    ...
    if (!initial) { try { initial = normalizeCode(window.localStorage.getItem(STORAGE_KEY)); } catch {} }
    initial = initial || "KE";
    setSelected(initial);
    try { window.localStorage.setItem(STORAGE_KEY, initial); } catch {}
    const timer = window.setTimeout(() => {
      try { window.dispatchEvent(new CustomEvent("anydaywork:country-changed", { detail: { countryCode: initial } })); } catch {}
    }, 0);
    return () => {
  ...
  useEffect(() => {
    if (pathname !== "/") return;
    let active = true;
    const base = globalCountries();
    setCountries(base);

    async function loadCountries() {
      const { data, error } = await supabase.from("demo_profiles").select("country_code,country_name");
      if (!active || error) return;
      const map = new Map(base.map(country => [country.code, country.name]));
  ...
  function changeCountry(code: string) {
    ...
    try { window.dispatchEvent(new CustomEvent("anydaywork:country-changed", { detail: { countryCode: normalized } })); } catch {}
  }
  ...
  return createPortal(
    <>
      <label className="anyday-country-picker" title="Choose marketplace country">
        ...
        <select value={selected} onChange={(event) => changeCountry(event.target.value)} aria-label="Marketplace country">
      ...
      <style jsx global>{`
        ...
        .anyday-country-picker { height:40px; display:flex; align-items:center; gap:6px; padding:0 9px; border:1px solid #d8e2da; border-radius:10px; background:#fff; color:#17221b; font:600 12px/1 system-ui,sans-serif; }
        .anyday-country-picker select { width:142px; max-width:22vw; border:0; outline:0; background:transparent; color:inherit; font:inherit; cursor:pointer; }
        @media (max-width:800px) { .anyday-country-picker { height:40px; padding:0 7px; gap:4px; } .anyday-country-picker select { width:112px; max-width:31vw; font-size:11px; } }
        @media (max-width:380px) { .anyday-country-picker select { width:92px; } }
      `}</style>
    </>, mount
  );
}
