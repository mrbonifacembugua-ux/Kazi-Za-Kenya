    ...
    return (
      <main className="page">
        <section className="card success">
          ...
          <p>Your service is now connected to your Kazi za Kenya account. Customers can find your service and you can respond to jobs using the same signed-in identity.</p>
  ...
  return (
    <main className="page">
      <section className="card">
        ...
        <p className="intro">Create a clear worker profile so customers can see what you do, view your previous work and contact you directly.</p>

        <form onSubmit={submit}>
          <label>Your name<input required name="name" placeholder="e.g. Peter Kamau" /></label>
          <label>Service or category<input required name="service" placeholder="e.g. Plumbing, painting, cleaning" /></label>
          <label>About your service<textarea required name="description" rows={5} placeholder="Tell customers what you do and the kind of work you handle" /></label>
          <div className="two">
            <label>Area<input required name="area" placeholder="e.g. Kilimani" /></label>
            <label>Starting price or price range<input required name="price" placeholder="e.g. From KSh 1,500" /></label>
          </div>
          <label>Availability<select required name="availability" defaultValue="AVAILABLE"><option value="AVAILABLE">Available for work</option><option value="BUSY">Currently busy</option></select></label>
          <label>Profile photo <small>(recommended)</small><input className="file" type="file" accept="image/*" onChange={(e) => setProfilePhoto(e.target.files?.[0] || null)} /></label>
          {profilePreview && <div className="profilePreview"><img src={profilePreview} alt="Profile preview" /><span>{profilePhoto?.name}</span></div>}
          <label>Proof of work / portfolio photos <small>(optional, up to 21)</small><input className="file" type="file" accept="image/*" multiple onChange={(e) => chooseWorkPhotos(e.target.files)} /></label>
          {workPhotos.length > 0 && <><div className="photoNote">📷 {workPhotos.length} portfolio photo{workPhotos.length === 1 ? "" : "s"} selected{workPhotos.length === 21 ? " · Maximum reached" : ""}</div><div className="gallery">{workPreviews.slice(0, 6).map((src, index) => <div className="thumb" key={src}><img src={src} alt={`Work preview ${index + 1}`} /></div>)}</div>{workPhotos.length > 6 && <div className="morePhotos">+ {workPhotos.length - 6} more photos in your portfolio</div>}</>}
          {(profilePhoto || workPhotos.length > 0) && <div className="notice">Photo upload storage is being connected separately. Your worker account and service details will be saved now; selected photos are not uploaded yet.</div>}
          <div className="notice">Kazi za Kenya connects you with customers. You and the customer discuss timing, final price, materials and other arrangements directly.</div>
          {errorMessage && <div className="error">{errorMessage}</div>}
          <button className="submit" type="submit" disabled={saving}>{saving ? "Creating profile..." : "Create my worker profile"}</button>
        </form>
      </section>
      <style jsx>{styles}</style>
    </main>
