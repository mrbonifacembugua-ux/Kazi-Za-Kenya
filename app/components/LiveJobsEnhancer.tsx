...
export default function LiveJobsEnhancer() {
  useEffect(() => {
    ...
    async function loadJobs() {
      ...
      const heading = Array.from(panel.querySelectorAll(".section-title")).find((el) =>
        (el.textContent || "").includes("Jobs people need done")
      );

      // When the user switches back to Find a worker, remove anything this helper added.
      if (!heading) {
        panel.querySelector(".live-jobs-from-db")?.remove();
        return;
      }
      ...
      const currentHeading = Array.from(panel.querySelectorAll(".section-title")).find((el) =>
        (el.textContent || "").includes("Jobs people need done")
      );
      if (!currentHeading) {
        panel.querySelector(".live-jobs-from-db")?.remove();
      ...
      if (!live) {
        ...
        currentHeading.insertAdjacentElement("afterend", live);
      }

      const profileMap = new Map((profiles || []).map((p) => [p.id, p.full_name || "Kazi za Kenya user"]));
      const photoMap = new Map<string, string[]>();
      (photos || []).forEach((p) => {
        const a = photoMap.get(p.job_id) || [];
        if (p.photo_url) a.push(p.photo_url);
        photoMap.set(p.job_id, a);
      });

      live.innerHTML = jobs.map((job) => {
        const jp = photoMap.get(job.id) || [];
        const photo = jp[0] || "https://images.unsplash.com/photo-1581578731548-c64695cc6952?auto=format&fit=crop&w=900&q=80";
