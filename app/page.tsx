...
export default function Home() {
  ...
  const [viewMode, setViewMode] =
    ...
    const [postedJobs, setPostedJobs] = useState<Job[]>(jobs);
...
const [jobLatitude, setJobLatitude] = useState<number | null>(null);
const [jobLongitude, setJobLongitude] = useState<number | null>(null);
...
const sendMessage = () => {
  ...
  if (!cleanSearch) return postedJobs;
...
useEffect(() => {
  const loadPostedJobs = async () => {
    try {
      ...
      const realJobs: Job[] = data.map((job: any) => ({
        id: job.id,
        customerName: "Kazi za Kenya user",
        customerEmoji: "👤",
        service: job.category || job.title,
        area:
          job.area ||
          job.location_text ||
          "Nairobi",
        ...
        budget:
          job.budget_min === job.budget_max
            ? `KSh ${job.budget_min}`
            : `KSh ${job.budget_min} - KSh ${job.budget_max}`,
        urgency: "FLEXIBLE",
        status:
          job.status === "taken"
            ? "WORKER FOUND"
      ...
      setPostedJobs((currentJobs) => {
        const demoJobs = jobs.filter(
          (demoJob) =>
            !realJobs.some(
              (realJob) =>
                realJob.id === demoJob.id
            )
        );

        return [...realJobs, ...demoJobs];
      });
    } catch (error) {
      console.error(
      ...
      {activeForm === "need" && (
        <div className="form-overlay">
          <div className="form-modal">
            ...
            <p>
              Tell nearby workers what you need done.
            </p>
...
<label className="photo-upload-label">
  📷 Photos of what needs to be done
  ...
  <input
    ...
    onChange={(e) => {
      ...
      // Photos will be connected to the job posting next.
      console.log("Selected job photos:", files);
    }}
  />
</label>
            <input
  ...
  try {
    ...
    if (results.length > 0) {
      setJobLatitude(Number(results[0].lat));
      setJobLongitude(Number(results[0].lon));
    }
  } catch (error) {
      ...
      async (position) => {
        ...
        try {
          ...
          if (response.ok) {
            ...
            setJobLocation(
              `${readableLocation}, Nairobi`
            );
          }
        } catch (error) {
          console.error(
            "Could not identify current area:",
            error
          );

          setJobLocation("Current location");
        }
      },
      (error) => {
        if (error.code === 1) {
...
/>
  ...
  onClick={async () => {
  if (!jobTitle.trim()) {
    alert("Please tell us what you need done.");
    return;
  }
    ...
    const { data: savedJob, error: jobError } =
      await supabase
        ...
        .insert({
          ...
          budget_min: budgetNumber,
budget_max: budgetNumber,
          location_text: jobLocation.trim(),
      county: "Nairobi",
  ...
  longitude: jobLongitude,
    ...
    if (jobError) {
      console.error("Job posting error:", jobError);
      alert(
        `Could not post the job: ${jobError.message}`
      );
      return;
    }
