...
export default function MarketplaceLiveJobs() {
  ...
  useEffect(() => {
    ...
    function captureSearch(event: Event) {
      ...
      if (placeholder.includes("search") || placeholder.includes("service") || placeholder.includes("area")) {
    ...
    return () => {
      stopped = true;
      observer?.disconnect();
      document.removeEventListener("input", captureSearch, true);
    };
  }, [pathname]);
  ...
  useEffect(() => {
    ...
    async function loadJobs() {
      ...
      const { data, error } = await supabase
        .from("jobs")
        .select("id,title,description,category,budget_min,budget_max,county,area,road,location_text,latitude,longitude,created_at,customer_id,customer:profiles!jobs_customer_id_fkey(full_name)")
        .eq("status", "available")
      ...
      if (error) {
        ...
        setError(null);
        setJobs((data || []) as unknown as LiveJob[]);
      }
      setLoading(false);
    }
    ...
    const channel = supabase
      .channel("kzk-marketplace-live-jobs")
  ...
  useEffect(() => {
    ...
    const legacyCards = Array.from(parent.children).filter(
      (el) => el !== mount && (el as HTMLElement).classList?.contains("job-card")
    ) as HTMLElement[];

    if (!loading && !error && jobs.length > 0) {
      legacyCards.forEach((card) => {
        card.dataset.kzkLegacyHidden = "1";
        card.style.display = "none";
      });
    } else {
      legacyCards.forEach((card) => {
        if (card.dataset.kzkLegacyHidden === "1") {
          card.style.removeProperty("display");
          delete card.dataset.kzkLegacyHidden;
        }
      });
    }
  ...
  }, [mount, jobs.length, loading, error]);

  const visible = useMemo(() => {
    const q = search.toLowerCase().trim();
    if (!q) return jobs;
    return jobs.filter((job) =>
      `${job.title} ${job.description} ${job.category || ""} ${job.area || ""} ${job.road || ""} ${job.location_text || ""} ${job.county || ""}`
        .toLowerCase()
        .includes(q)
    );
  }, [jobs, search]);
    ...
    <>
      <div className="kzk-live-wrap">
        ...
        {!loading && !error && visible.map((job) => (
          <button className="kzk-live-card" type="button" key={job.id} onClick={() => setSelected(job)}>
            <div className="kzk-live-icon">🛠️</div>
            <div className="kzk-live-main">
              <div className="kzk-live-top"><strong>{job.title}</strong><span>OPEN</span></div>
              <div className="kzk-live-meta">{job.category || "General work"} · {job.area || job.location_text || job.county}</div>
              <div className="kzk-live-bottom"><b>{budget(job)}</b><span>{when(job.created_at)}</span></div>
            </div>
          </button>
        ))}
      </div>
      ...
      {selected && createPortal(
        <div className="kzk-live-modal-backdrop" role="presentation" onMouseDown={(e) => { if (e.currentTarget === e.target) setSelected(null); }}>
          <section className="kzk-live-modal" role="dialog" aria-modal="true" aria-label="Job details">
            <button className="kzk-live-close" type="button" onClick={() => setSelected(null)} aria-label="Close">×</button>
            <div className="kzk-live-modal-tag">LIVE JOB · OPEN</div>
            <h2>{selected.title}</h2>
            <div className="kzk-live-modal-sub">📍 {selected.area || selected.location_text || selected.county}{selected.road ? ` · ${selected.road}` : ""}</div>
            <div className="kzk-live-budget">{budget(selected)}</div>
            <p>{selected.description}</p>
            <div className="kzk-live-customer">Posted by <strong>{selected.customer?.full_name || "Kazi za Kenya customer"}</strong> · {when(selected.created_at)}</div>
            <button className="kzk-live-action" type="button" onClick={async () => {
              const { data } = await supabase.auth.getUser();
              if (!data.user) {
                window.location.href = "/login";
                return;
              }
              window.alert("This job is live. Worker response and messaging is the next connection being added.");
            }}>I’m interested in this job</button>
          </section>
        </div>,
        document.body
      )}
