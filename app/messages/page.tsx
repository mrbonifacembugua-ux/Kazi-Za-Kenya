...
export default function MessagesPage() {
  ...
  const person = params.get("with") || "Kazi za Kenya user";
  const [messages, setMessages] = useState<string[]>([]);
  const [text, setText] = useState("");

  useEffect(() => {
    const key = `kazi-chat-${person}`;
  ...
  return (
    <main className="page">
      ...
      <p className="note">This is the working browser chat. Messages are saved on this device while the full account-to-account messaging backend is connected.</p>
