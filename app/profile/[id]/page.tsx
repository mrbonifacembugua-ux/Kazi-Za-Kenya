...
export default function ProviderProfile(){
  ...
  useEffect(()=>{(async()=>{try{
    const {data:p,error:pe}=await supabase.from("profiles").select("id,full_name,phone,county,area,road,bio,latitude,longitude,profile_photo_url,verification_status").eq("id",id).eq("role","provider").eq("is_active",true).maybeSingle();
    if(pe) throw pe; if(!p) throw new Error("This provider could not be found.");
    const [{data:s,error:se},{data:ph,error:phe}]=await Promise.all([
    ...
    if(se) console.warn("Services could not be loaded:",se.message);
    if(phe) console.warn("Portfolio could not be loaded:",phe.message);
    setProfile(p); setServices(s||[]); setPhotos((ph||[]).filter(x=>x.photo_url));
  }catch(e){setError(e instanceof Error?e.message:"Profile could not be loaded.")}finally{setLoading(false)}})()},[id]);

  async function sendMessage(){
    setMessageError(""); const text=message.trim(); if(!text){setMessageError("Please write a message first.");return;} setSending(true);
    try{
      const {data:{user}}=await supabase.auth.getUser();
      if(!user){router.push(`/login?next=/profile/${encodeURIComponent(id)}`);return;}
      ...
      if(!conversationId){const {data:conversation,error:ce}=await supabase.from("conversations").insert({employer_id:user.id,provider_id:id}).select("id").single();if(ce)throw ce;conversationId=conversation.id;}
      const {error:me}=await supabase.from("messages").insert({conversation_id:conversationId,sender_id:user.id,body:text}); if(me)throw me;
      setMessage("");setSent(true);setShowMessage(false);setTimeout(()=>setSent(false),3500);
    }catch(e){setMessageError(e instanceof Error?e.message:"Message could not be sent. Please try again.")}finally{setSending(false)}
  }

  function findOnMap(){
    const q=new URLSearchParams(); q.set("provider",id); if(profile?.area)q.set("area",profile.area); if(profile?.road)q.set("road",profile.road);
    if(profile?.latitude!=null && profile?.longitude!=null){q.set("lat",String(profile.latitude));q.set("lng",String(profile.longitude));}
