...
export default function RespondToJobPage() {
  ...
  useEffect(() => {
    ...
    setJobId(id);
    async function load() {
      if (!id) { setError("No job was selected."); setLoading(false); return; }
      const { data: auth } = await supabase.auth.getUser();
      if (!auth.user) { router.push(`/login?next=${encodeURIComponent(`/jobs/respond?job=${id}`)}`); return; }
      const { data, error } = await supabase
        .from("jobs")
        .select("id,title,description,category,county,area,budget_min,budget_max,status,customer_id")
        .eq("id", id)
        .single();
      if (error || !data) setError(error?.message || "Job not found."); else setJob(data);
      setLoading(false);
    }
    load();
  }, [router]);
  ...
  async function submit(e: FormEvent) {
    ...
    if (!user) { router.push(`/login?next=${encodeURIComponent(`/jobs/respond?job=${jobId || ""}`)}`); return; }
    ...
    if (insertError) setError(insertError.code === "23505" ? "You have already responded to this job." : insertError.message); else setDone(true);
    setSending(false);
  }
  ...
  return <main className="page"><section className="card"><button className="back" onClick={() => router.back()}>← Back</button><div className="brand">Any<span>DayWork</span></div><h1>Respond to this job</h1>{job && <div className="job"><b>{job.title}</b><span>{job.category} · {job.area || job.county || "Location not set"}</span><p>{job.description}</p></div>}{error && <div className="error">{error}</div>}{job && <form onSubmit={submit}><label>Message to employer<textarea rows={5} value={message} onChange={e=>setMessage(e.target.value)} placeholder="Briefly explain how you can help" /></label><label>Your quoted price <small>(optional)</small><input inputMode="numeric" value={price} onChange={e=>setPrice(e.target.value)} placeholder="e.g. 2500" /></label><div className="notice">The employer chooses who to hire. AnyDayWork connects you; you discuss final price, timing and materials directly.</div><button className="submit" disabled={sending || job.status !== "available"}>{sending ? "Sending…" : "I'm interested in this job"}</button></form>}<style jsx>{styles}</style></section></main>;
}
