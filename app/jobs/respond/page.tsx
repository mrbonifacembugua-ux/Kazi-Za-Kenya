...
export default function RespondToJobPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const { data, error } = await supabase.from("jobs").select("id,title,description,category,area,location_text,budget_min,budget_max,status,customer_id").eq("id", id).single();
